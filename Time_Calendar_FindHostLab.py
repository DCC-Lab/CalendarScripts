...
cicchettiLab=[]
for i in range(len(members)):
    if isinstance(members.at[i, 'cicchetti'], str):
        cicchettiLab.append(members.at[i, 'cicchetti'])
for i in range(len(userlist)) :
    matching2 =[]
    print( userlist[i])  
    if isinstance(userlist[i], str): #to avoid the NaN        
        if any(userlist[i] in s for s in cicchettiLab):
            matching2 = [s for s in cicchettiLab if userlist[i] in s]
            L[i]=('Cicchetti')      
                      
TimofeevLab=[]
for i in range(len(members)):
    if isinstance(members.at[i, 'Timofeev'], str):
        TimofeevLab.append(members.at[i, 'Timofeev'])
for i in range(len(userlist)) :
    matching2 =[]
    print( userlist[i])  
    if isinstance(userlist[i], str): #to avoid the NaN        
        if any(userlist[i] in s for s in TimofeevLab):
            matching2 = [s for s in TimofeevLab if userlist[i] in s]
            L[i]=('Timofeev')  

EthierLab=[]
for i in range(len(members)):
    if isinstance(members.at[i, 'Ethier'], str):
        EthierLab.append(members.at[i, 'Ethier'])
for i in range(len(userlist)) :
    matching2 =[]
    print( userlist[i])  
    if isinstance(userlist[i], str): #to avoid the NaN        
        if any(userlist[i] in s for s in EthierLab):
            matching2 = [s for s in EthierLab if userlist[i] in s]
            L[i]=('Ethier') 
            
LevesqueLab=[]
for i in range(len(members)):
    if isinstance(members.at[i, 'Levesque'], str):
        LevesqueLab.append(members.at[i, 'Levesque'])
for i in range(len(userlist)) :
    matching2 =[]
    print( userlist[i])  
...
os.chdir(directory)
# Read excel file from microscope
#file = 'Levesque-ZeissLSM700_Users.xlsx'
with pd.ExcelWriter(file[:-5] + '_user.xlsx') as writer:
