...
os.chdir("Y:\Simon\Platforme_Imagerie\GoogleCalendar_time\Levesque-ZeissLSM700")
...
Labs = ["Levesque", "Parent","Deschenes", "PDK" , "YDK", "Toth","SAG","JK","CFS","JPJ","Labonte", "Menard","cicchetti"]

#%%
yeardata = []
for i in range(len(data)):
    if '.2018 'in data.loc[i,'Start']:
        yeardata.append(data.loc[i])
#%%
singleday=1
for findmonth in range(1,3):
    MonthData = []
    for i in range(len(data)):
...
#        plt.close(1)
    ...
    for day in range(1,len(MonthData)):
    #    day = 25
        reservations=[]
        for i in range(len(MonthData)):
...
plt.xticks(np.arange(1,len(yeardata)), np.arange(1,len(yeardata)))
